...
**HTM-008**: The system MUST implement a Django middleware class, registered through `settings.MIDDLEWARE`, serving both synchronous and asynchronous Django deployments through exactly one class. It MUST mark itself async-capable with a stdlib-only coroutine-function shim and MUST NOT import `django` or `asgiref` at `semlog` import time. It MUST bind per-request trace/baggage/identity context from inbound headers per TCP-001 to TCP-009, and MUST reproduce, unchanged, the `full`/`hybrid`/`off` mode guarantees already defined for `WSGIMiddleware`/`ASGIMiddleware` (LM-003, LM-005): a synchronous view and an `async def` view registered through the same class MUST produce records with identical field shape from the same inbound `traceparent`, and in `off` mode the class MUST produce no record, field or side effect of its own while the wrapped view still runs. Test: `tests/test_django_matrix.py::DjangoMiddlewareRegistrationTests`, `::DjangoMiddlewareAsyncMarkingTests`, `::DjangoMiddlewareModeTests`; `tests/test_packaging_hygiene.py::SemlogImportsWithoutDjangoTests`.

> **Design decision (HTM-008).** Rationale: one class mirrors Django's own dual sync/async middleware idiom, so it composes exactly like any other `MIDDLEWARE` entry; deferring the Django/asgiref import keeps `semlog` importable with zero runtime dependencies (LP-001) where Django is absent. Rejected alternative: two separate classes, which would double the public surface and diverge from the single-entry `MIDDLEWARE` convention.

**HTM-009**: Documentation MUST recommend outermost placement of the class in `settings.MIDDLEWARE` by default, matching the existing WSGI/ASGI outermost guidance, and MUST document the tradeoff, in parity between README.md and README.es.md: outermost placement maximizes context coverage of other middlewares' own logging but MAY lose exception visibility to a competing `process_exception` closer to the view (HTM-012); closest-to-view placement maximizes exception-capture priority but narrows context coverage. Test: `tests/test_readme_i18n.py::ReadmeFrameworkRecipesParityTests`, `tests/test_agent_docs_anti_drift.py`.

> **Design decision (HTM-009).** Rationale: Django's exception-middleware loop stops at the first non-`None` `process_exception` result, making placement a real tradeoff that must be documented, not hidden. Rejected alternative: picking one placement silently, which would surprise an adopter already using another exception-handling middleware.

**HTM-010**: Using the class together with `WSGIMiddleware` or `ASGIMiddleware` wrapping the same application MUST NOT be blocked at runtime, and MUST be documented as unsupported: each layer parses the inbound `traceparent` independently and mints its own `span_id`, so with `log_requests=True` on both, two `http.server.request` events are emitted per request, sharing the same `trace_id` with different `span_id` values, and neither layer raises. Test: `tests/test_django_matrix.py::DjangoMiddlewareCoexistenceTests`.

> **Design decision (HTM-010).** Rationale: `contextvars` token-based reset already nests safely, so a runtime block would be unnecessary; matches TCP-011's existing coexistence-with-another-propagator precedent. Rejected alternative: raising at configuration time, which would penalize a transitional migration from the wrap-the-app recipe.

**HTM-011**: `process_exception(self, request, exception)` MUST stash the exception information without swallowing it and MUST always return `None`, leaving Django's own exception handling unmodified, in every case. After `get_response` (or its async equivalent) returns and the final response is known, the middleware MUST emit the request event through the existing, unchanged `_emit_request_event` mechanism (HTM-007), applying its existing INFO/ERROR selection rules against the real final `status_code`: an unhandled view exception MUST produce one ERROR `http.server.request` record with `exception.stacktrace` and the real final `status_code`, while Django's own error handling still produces its response ([OTEL-SEMCONV]). This event MUST be enabled by the Django setting `SEMLOG_LOG_REQUESTS` (default off, resolved once at construction, the same event HTM-007 already defines); an explicit `log_requests=` keyword MUST always win over the setting, and a non-boolean setting value MUST raise `ValueError` naming the value and its source. Test: `tests/test_django_matrix.py::DjangoMiddlewareExceptionTests`, `::DjangoLogRequestsSettingTests`.

Backing: External: [OTEL-SEMCONV]

**HTM-012**: `process_exception` MUST be documented as permanently unable to observe an exception raised by another middleware's own code: when a middleware registered above the class raises inside its own code, not the view, semlog's `process_exception` MUST NOT be called and semlog captures no traceback for that failure. It MUST also be documented as pre-emptable by another registered middleware's own `process_exception` closer to the view, which stops Django's exception-middleware loop before semlog's hook runs, so semlog's own `process_exception` is never invoked for that request. Neither limitation MUST be treated as a defect. Test: `tests/test_django_matrix.py::DjangoMiddlewareExceptionVisibilityTests`.

> **Design decision (HTM-012).** Rationale: Django wraps every middleware individually in `convert_exception_to_response` before semlog's hook runs, and stops its exception-middleware loop at the first non-`None` result; observing every middleware's own exception is impossible without changing Django itself. Rejected alternative: monkeypatching upstream middleware exception handling, which would exceed the library's non-invasive, zero-dependency design.

**HTM-013**: The bound trace/baggage/identity context MUST stay correctly bound throughout a Django `StreamingHttpResponse` body, for both a synchronous iterable and an asynchronous iterable, matching the existing streaming guarantee (HTM-003), and MUST be verified on Django 3.2.9, the oldest supported row (CP-002): every log emitted while consuming the body, after the middleware chain itself has already returned, MUST still carry the request's `trace_id`/`span_id`. A `FileResponse` MUST be left unrebound, since rebinding its `streaming_content` would disable the underlying sendfile path. Test: `tests/test_django_matrix.py::DjangoMiddlewareStreamingTests`, `::DjangoMiddlewareAsyncStreamingTests`, `::DjangoMiddlewareFileResponseTests`.

> **Design decision (HTM-013).** Rationale: HTM-003 already guarantees streaming context parity at the WSGI/ASGI protocol level; `StreamingHttpResponse` sits above that protocol, so the same guarantee needs its own rebinding for a view that returns a generator instead of a complete body. Rejected alternative: popping context immediately after `get_response` returns, which breaks the guarantee the moment the body is a generator consumed after the middleware chain unwinds.

**HTM-014**: The system MUST implement an aiohttp middleware, registered as an `aiohttp.web.Application(middlewares=[...])` entry, declaring aiohttp's new-style marker `__middleware_version__ = 1`, and MUST NOT import `aiohttp` at `semlog` import time. Before calling the handler it MUST parse the inbound trace/baggage headers and bind a request-scoped context per TCP-001 to TCP-009, and it MUST reset that context on every exit path: a returned response, an exception of any type, and task cancellation. Because aiohttp serves every request of one keep-alive connection from a single task, and therefore from a single `contextvars.Context`, a later request on that connection MUST NOT observe the previous request's `trace_id`, `span_id` or baggage ([PEP567]). The context MUST stay bound for everything the handler itself awaits, including `StreamResponse` writes and a WebSocket session. Test: `tests/test_aiohttp_middleware.py::AiohttpMiddlewareContextTests`.

**HTM-015**: The middleware MAY emit the single `http.server.request` completion event HTM-007 already defines, disabled by default and enabled only with `log_requests=True`, carrying exactly the fields HTM-007 defines and never `url.query`; in `hybrid` mode it MUST route to JSON only (LM-003). Its `http.response.status_code` MUST be read from the object the handler returned in a way that cannot raise: a handler that returns no response MUST still reach aiohttp's own diagnostic, and MUST NOT have an exception raised inside semlog reported in its place as this request's failure. An exception leaving the handler MUST be classified by its type, never by a duck-typed `status` attribute: only an `aiohttp.web.HTTPException` MUST produce an INFO event carrying that exception's own status, since aiohttp turns it into the response it sends; every other exception MUST produce an ERROR event with the rendered traceback and no status, `ConnectionResetError` and `BrokenPipeError` included, because aiohttp answers 500 for them. `asyncio.CancelledError` MUST be the only exception that produces no event at all. The middleware MUST NOT swallow or alter any exception. Test: `tests/test_aiohttp_middleware.py::AiohttpMiddlewareRequestEventTests`, `::AiohttpHybridRequestEventTests`.

> **Design decision (HTM-015).** Rationale: an outbound `ClientResponseError` carries a `status` of the call the handler made, not of this request, so a duck-typed check reports a 404 for a request aiohttp answers 500 and silently drops the traceback; classifying by `aiohttp.web.HTTPException` reports exactly what aiohttp will send, and a peer disconnect is a real 500 the operator needs, not noise to suppress. Rejected alternative: keeping the duck-typed `status` check and the blanket suppression of connection errors, which makes the event disagree with the response the client received.

**HTM-016**: In `off` mode the middleware MUST behave as if semlog were not installed (LM-005): it MUST NOT parse inbound headers, MUST NOT push a scope, and MUST NOT emit the completion event under any configuration, including `log_requests=True`. The handler MUST still run and its response MUST still be returned unchanged, and an exception it raises MUST still propagate untouched; `inject()` and `bind()` called from inside the handler MUST be inert, exactly as they are outside any middleware. Test: `tests/test_aiohttp_middleware.py::AiohttpMiddlewareOffModeTests`.

> **Design decision (HTM-016).** Rationale: `off` is the rollback switch, so it must be inert inside a request too, matching the guarantee `WSGIMiddleware`, `ASGIMiddleware` and `DjangoMiddleware` already give; anything less would leave a service that "turned semlog off" still paying for header parsing and still binding a scope. Rejected alternative: checking the mode only at emission time, which would keep parsing headers and pushing a scope for every request in a mode that promises no observable effect.

**HTM-017**: The completion event's `event.duration` MUST be documented, in parity between README.md and README.es.md and in the agent guide, as covering the handler's execution only. aiohttp sends the response after the middleware chain has returned, so response transmission, a deferred asynchronous response body and a `FileResponse` send are outside the measured interval, and a WebSocket session is inside it because the handler awaits it. This scope MUST NOT be treated as a defect. Test: `tests/test_aiohttp_middleware.py::AiohttpEventDurationScopeTests`.

> **Design decision (HTM-017).** Rationale: aiohttp gives a middleware no hook after transmission, so the honest choice is to measure what the middleware can actually see and say so, rather than publish a number whose meaning changes with the response type. Rejected alternative: claiming a full request-to-last-byte duration the middleware cannot observe, or patching the response object to learn when transmission ended, which would exceed the library's non-invasive design.
...
### 8.4 Celery integration

**CEL-001**: The system MUST expose `celery(app)` accepting a Celery `Celery` instance. `import semlog` MUST succeed with Celery absent from the environment. `celery(app)` MUST be idempotent across repeated calls, with the same app instance or a different one, and MUST NOT raise, whether called before or without `configure()`. No receiver `celery(app)` connects to a Celery signal MUST ever raise into Celery: Celery re-raises whatever a connected `import_modules` receiver raises, aborting worker/beat startup; its other signals only log a raising receiver's exception instead, but a receiver connected here MUST NOT depend on which behavior applies. Test: `tests/test_celery_wiring.py::ReceiversNeverRaiseTests`, `tests/test_celery_integration.py::EntryPointIdempotencyTests`, `tests/test_packaging_hygiene.py::SemlogImportsWithoutCeleryTests`.

> **Design decision (CEL-001).** Rationale: a Celery deployment that never calls `celery(app)`, or that runs with Celery entirely absent, must never be affected by importing semlog; a receiver able to abort worker or beat startup would turn this integration itself into a production risk. Rejected alternative: letting a receiver's exception propagate into Celery's own signal dispatch, which would tie semlog's own bugs to Celery process startup.

**CEL-004**: In `off` mode, `celery(app)` MUST produce no record, span, injected header, or flush side effect, matching LM-005. Test: `tests/test_celery_wiring.py::OffModeReceiverTests`, `tests/test_celery_integration.py::OffModeCeleryTests`.

> **Design decision (CEL-004).** Rationale: `off` mode is already a rollback switch (LM-005) that leaves no semlog-owned side effect running anywhere else in the library; the Celery integration honors the same guarantee instead of becoming a second, independently toggled kill switch that could drift from it. Rejected alternative: a Celery-specific enable/disable flag.

**CEL-005**: Each task execution MUST run inside an `operation()` span. When the publisher injected a `traceparent`, the task's span MUST continue that `trace_id` with a new `span_id`. A task that publishes a child task from inside its own execution MUST propagate its own span into that child's publish-side inject. Concurrent tasks MUST NOT share `span_id` ([TRACECONTEXT], [PEP567]). Test: `tests/test_celery_wiring.py::CarrierTests`, `::TaskSpanReceiverTests`, `tests/test_celery_integration.py::WorkerSpanTests`, `::EagerSpanTests`.

**CEL-006**: `celery(app)` MUST inject the current span's `traceparent`, and trusted *baggage* (TCP-008), into a task's outbound headers at publish time. With no active span, headers MUST be left unchanged ([TRACECONTEXT]). Test: `tests/test_celery_wiring.py::PublishReceiverTests`, `tests/test_celery_integration.py::PublishInjectTests`.

**CEL-002**: In `full` mode, after `celery(app)`, worker, beat and `celery.task`-logger records MUST be emitted as semlog JSON, exactly once each; Celery's own `--loglevel` MUST govern the effective level. "No Celery text output" (SC-1) means no Celery *log record* is rendered as text: a direct stream write bypassing the logging system entirely (the worker/beat banner, a shutdown notice such as `safe_say`, or a CLI warning) is out of scope, and on the Celery 5.2.7 row beat can print its own banner even with `-q`, sometimes with a JSON record glued onto the banner's last line -- a pre-existing Celery behavior, unrelated to semlog. Test: `tests/test_celery_wiring.py::FullModeFlagTests`, `::TaskLoggerCaptureTests`, `tests/test_celery_integration.py::ImportModulesFlagTests`, `tests/test_celery_matrix.py::FullModeWorkerTests`, `::FullModeBeatTests`.

> **Design decision (CEL-002).** Rationale: a worker/beat process that already calls `configure()` should not have to ALSO separately silence Celery's own text logging by hand; a lazy, per-app `worker_hijack_root_logger` write, made only when semlog genuinely owns root, keeps a user's own explicit hijack choice (plain `app.conf` or a namespaced Django setting) authoritative instead of silently overriding it in every case. Rejected alternative: an eager write at `celery(app)` call time, which would run before `configure()` has necessarily resolved the process's mode and would risk wiping a user-installed handler.

**CEL-003**: In `hybrid` mode, `celery(app)` MUST leave Celery's own text output byte-identical to a run without `celery(app)`, while still adding CEL-005/006/007 behavior. This guarantee is pinned to `--loglevel=INFO` and above (SC-2), after normalizing run-variant values (timestamps, UUIDs, memory addresses, durations, PIDs, `ForkPoolWorker-N` identifiers, and per-run temporary paths). At `DEBUG`, Celery's own `TaskPool: Apply ...` record prints the outbound message headers verbatim, including the `traceparent` (and any trusted *baggage*) CEL-006 injected at publish time -- a documented limitation, not a defect: baggage values reaching that line bypass semlog's own redaction, since Celery renders it directly, never through semlog's pipeline. Test: `tests/test_celery_wiring.py::HybridInertTests`, `tests/test_celery_matrix.py::HybridByteIdentityTests`.

> **Design decision (CEL-003).** Rationale: `hybrid` mode's entire purpose (LM-001/LM-003) is adopting semlog into a service with its own existing log lines, without changing anything already printed; the Celery integration keeps that promise instead of becoming an exception to it. Rejected alternative: capturing `celery.task`/root in hybrid too, which would defeat the byte-identity guarantee hybrid mode exists to provide. The DEBUG/baggage limitation is accepted, not designed around, because closing it would mean either never injecting in hybrid (losing CEL-006 entirely there) or patching Celery's own `TaskPool` logging, both rejected as disproportionate to a `DEBUG`-only, opt-in exposure.

**CEL-007**: On `worker_process_shutdown`, the system MUST flush every record enqueued before this child process exits ([QUEUEHANDLER]). A solo-pool process, which never sends `worker_process_shutdown` at all, MUST still have its records flushed on normal exit (the existing atexit-based drain, LP-011). The flush receiver MUST connect from inside the `worker_process_init` receiver itself, not at `celery(app)` call time, so it is guaranteed to run after any import-time receiver a task module connects to `worker_process_shutdown`. Test: `tests/test_celery_wiring.py::ShutdownFlushTests`, `tests/test_celery_integration.py::ChildFlushWiringTests`, `tests/test_celery_matrix.py::PreforkShutdownFlushTests`, `::SoloExitFlushTests`.
...
**CP-017**: The package source code MUST define no abstract base class (`abc.ABC`, `ABCMeta` or `typing.Protocol`) and MUST NOT create classes through factories (`collections.namedtuple`, `typing.NamedTuple` or three-argument `type()`), keeping the implementation free of indirect class layers that would make it harder to review as a single unit. This document does not publish a maximum source-line count or a maximum class count; the guardrail against unbounded growth is human review together with this requirement's surviving rules and CP-016's fixed three-stage architecture. Test: `tests/test_class_budget.py` (no-ABC and no-class-factory cases only).

(Previously: a maximum budget of 1500 non-blank, non-comment lines and at most 6 classes, enforced by `tests/test_source_budget.py` and `tests/test_class_budget.py::test_at_most_six_classes_total`, with flatness attributed to that class budget together with CP-016. Both numeric budgets, their enforcing tests, and `test_every_class_is_design_named_or_a_transport_subclass` are removed; flatness is now attributed to CP-016 and to this requirement's surviving no-ABC/no-factory rules alone, with no remaining reference to a class budget.)

> **Design decision (CP-017).** Rationale: the maintainer judged an idiomatic Django `MIDDLEWARE` class more valuable than a fixed line/class ceiling; the no-ABC and no-class-factory rules keep the type surface auditable without capping its size. Rejected alternative: raising the numeric budgets instead of removing them, which would still eventually force an artificial class split as the library grows.

**CP-015**: The public surface of the library MUST consist of exactly eleven names: `configure`, `WSGIMiddleware`, `ASGIMiddleware`, `DjangoMiddleware`, `AiohttpMiddleware`, `operation`, `bind`, `inject`, `flush`, `llm` and `celery`. All configuration MUST be expressed as keyword-only parameters of `configure()`; the system MUST NOT expose a configuration object or a configuration dictionary. One exception: the Django middleware's per-request event switch is read from the Django setting `SEMLOG_LOG_REQUESTS`, because Django instantiates a `settings.MIDDLEWARE` entry with a single positional argument and no keyword can reach it (HTM-008). It is a single boolean, not a configuration object or dictionary, and it adds no public name. A second exception: `celery(app, /)` (CEL-001) takes exactly one positional-only argument, the Celery app, and no keyword argument at all; it is likewise not a configuration object or dictionary, and it adds exactly the one public name already counted above. The event catalog MUST be accepted as a JSON document (data), not as a configuration object or class. Any addition to this surface MUST be a deliberate, SemVer-relevant, documented change ([SEMVER]). Test: task 3.17/4.3; `tests/test_public_surface.py`.

(Previously: exactly ten names, without `celery` and without the second, positional-only exception sentence; before that, exactly nine names, without `AiohttpMiddleware`. Also previously: exactly eight names, without `DjangoMiddleware`, and without the `SEMLOG_LOG_REQUESTS` exception sentence.)
...
> **Design decision (LM-002).** Rationale: a marked call made from framework startup hooks (for example `AppConfig.ready()` or a WSGI preload hook) must never crash the host process; arming the patch on the existing `Logger` class at import time, not inside `configure()`, guarantees this without adding an extra class for the sole purpose of the patch. Rejected alternative: arming inside `configure()`, which reproduces the exact preload-time crash observed with an unpatched `Logger._log`; or a `setLoggerClass()` custom Logger, which adds an avoidable class and misses loggers already obtained before the switch.
...
**CP-008**: Any tool, library or package outside the Python standard library, used anywhere in development, testing, building or benchmarking, MUST be: (a) from the standard library, (b) one of the test/benchmark subjects explicitly approved by the user (FastAPI, Starlette, Django, aiohttp, Celery, loguru, structlog), each of which is installed only to run tests or benchmarks and MUST NOT become a runtime or an optional-extra dependency of the published distribution, (c) `ruff` (approved for lint and format, locally and in continuous integration), (d) GitHub Actions (approved for continuous integration), (e) the approved build backend (section 11.1), (f) the `uv` CLI (Astral), approved as the build frontend and development environment manager (`uv build`, `uv sync`, `uv run`), used in development and in continuous integration, never at runtime, or (g) a tool explicitly approved by the user before its use. `mypy`, `coverage.py`, `jsonschema`, `gunicorn` and `uvicorn` are NOT approved; where their function would be needed, the standard library alternative is used (the in-house JSON Schema subset validator instead of `jsonschema`; no third-party coverage tool; instead of `gunicorn` and `uvicorn`, the tests exercise the middlewares through the standard library's `wsgiref` for WSGI (CP-010) and a hand-written ASGI `scope`/`receive`/`send` caller for ASGI (CP-011)). The exception for the build backend and frontend exists because `distutils` was removed from the standard library in Python 3.12 ([PEP632]); the backend is declared through the PEP 517 interface and the PEP 518 `[build-system]` table ([PEP517], [PEP518]). Celery is approved as a compatibility-matrix test subject only (CP-020); it MUST NOT become a runtime or build dependency, and none of Celery's own dependencies (for example its broker-client library) are separately approved or pinned by this document. Test: task 4.2/4.7.

(Previously: the approved subject list named neither aiohttp nor Celery, did not state that an approved subject may not become a runtime or optional-extra dependency, and said nothing about Celery's own dependencies.)
...
**CP-013**: The only third-party packages usable anywhere in the project are: FastAPI, Starlette, Django, aiohttp, Celery (compatibility matrix subjects, installed for tests only, never declared as a runtime dependency or a package extra); `loguru`, `structlog` (benchmark comparison subjects); `ruff` (lint and format tool); and the approved build backend together with its frontend CLI (`uv_build` and the `uv` CLI, section 11.1), needed because `distutils` was removed from the standard library in Python 3.12 ([PEP632]) and every build backend is declared according to PEP 517 and PEP 518 ([PEP517], [PEP518]). Any other third-party package MUST NOT be introduced without new explicit approval from the maintainer; this includes Celery's own dependencies (for example its broker-client library), which stay unpinned and unnamed by this document (CP-020, CP-008). Test: task 4.2.

(Previously: the compatibility-matrix subjects were FastAPI, Starlette and Django only, with no statement about package extras, and the list named neither aiohttp nor Celery.)

**CP-020**: Continuous integration MUST run the automated suite against Celery 5.2.7 (Python 3.10, floor row) through Celery 5.6.3 (across the supported Python range), using an in-memory broker with no external broker service, mirroring CP-002's FastAPI/Django matrix shape ([PYVERSIONS]). Each row also runs a Django version, for the Django-layout subprocess fixture. Test: `tests/test_repository_policy.py::CeleryMatrixJobTests`, `tests/test_celery_matrix.py`.

(Previously: the Test sentence named only `CeleryMatrixJobTests`, since `tests/test_celery_matrix.py` did not exist yet; no Django version was declared per row.)
...
**DOC-010**: The system MUST publish an `AGENTS.md` file at the repository root, written in English and conforming to the agents.md convention, aimed at coding agents that CONTRIBUTE to SEMLOG, not at call sites that consume the published library. It MUST state the exact commands to set up the environment (`uv sync`), run the test suite (`uv run python -m unittest discover`), lint (`uv run ruff check`), check formatting (`uv run ruff format --check`) and build the package with the approved `uv_build` backend through the uv CLI (`uv build`); it MUST state the `pip`-based fallback for each of these commands when `uv` is unavailable (`pip install -e .` for setup; `python -m unittest discover`, `ruff check` and `ruff format --check` once installed; `python -m pip wheel . --no-deps --wheel-dir dist` for the build); the project's hard rules (standard-library-only tooling per CP-008, zero runtime dependencies, the flat three-stage architecture per CP-016, and the no-abstract-class/no-class-factory rules together with the eleven-name public surface per CP-017/CP-015); the strict TDD workflow; the list of approved third-party packages (CP-013), naming aiohttp and Celery among the test-only compatibility-matrix subjects and stating that no approved subject ever becomes a runtime dependency or a package extra; the prohibition of any company or vendor name in code or defaults; and references to STANDARDS.md and to the project specification ([AGENTSMD]). Test: task 1.12/4.19.

(Previously: a nine-name public surface, then a ten-name one, and an approved-package list that named neither aiohttp nor Celery and did not mention package extras.)

**DOC-011**: The system MUST include a single, self-sufficient agent guide, as a Markdown document written in English, that lets a coding agent apply SEMLOG correctly at the call site OFFLINE, without network access. It MUST be included INSIDE the distributed package as package data, so that its content always stays paired with the installed library version; it is the ONLY agent-facing document included in the distributed wheel (`llms.txt`, `AGENTS.md`, this document (STANDARDS.md), README.md and the JSON schemas remain in the repository only). The stored guide file MUST NOT contain the literal of the installed SEMLOG version or of the pinned OTel semconv version; instead, the OUTPUT of `semlog.llm()` and `python -m semlog llm` (CP-018) MUST be preceded by a header, generated at read time (no cache), that states the installed SEMLOG version, read from the installed distribution's metadata through the standard library's `importlib.metadata` (the same lookup used for `telemetry.sdk.version`), and the pinned OTel semconv version, read from a single constant in the package source code. Its content MUST include: the complete public API surface (each public name with its signature); the call-site MUST/MUST NOT rules (static event names per the event name grammar; variables passed only through `extra`, under the configured custom namespace or a declared OTel semconv key; `logger.exception` called exactly once at the boundary that handles the exception; no f-string, dictionary interpolation or `json.dumps` used to build the log message; no secret or request/response body logged; no call to `print`); the level-to-severity semantics table (per DOC-006); a summary of the output contract with at least one real single-line JSON example; the configuration parameters and their environment variable precedence, including the `mode` parameter (its three values, resolution precedence, and that an invalid value raises `ValueError`) and the `semlog=True` keyword (accepted in every mode once `semlog` is imported, its per-mode effect, and the hybrid non-stream-handler limitation of LM-004); the consequence of `hybrid` leaving the root logger's level untouched (LP-010), namely that a process which has not set a root level filters out every `INFO` record marked `semlog=True`, including the `INFO` `http.server.request` completion event, that `configure(level=...)` does not change this, and the remedy the application applies instead; every supported integration route for FastAPI (`app.add_middleware(semlog.ASGIMiddleware, ...)` beside the wrapper recipe, each with its exception-handler caveat, and the wrapper recipe registering its routes before the wrapping line) and for Django (the `settings.MIDDLEWARE` recipe for `DjangoMiddleware`, with `configure()` placed in `AppConfig.ready()`, and the `WSGIMiddleware`/`ASGIMiddleware` wrapper around `get_wsgi_application()`/`get_asgi_application()`, with the note that Django converts a view exception into a 500 response before the wrapper sees it, so that route cannot attach the traceback); the Celery integration route (`semlog.celery(app)`, called once in the module that creates `app`, restating that Django keeps `configure()` in `AppConfig.ready()` rather than in the Celery app-creation module), including the `full`-mode worker/beat/task logging guarantee, the `hybrid`-mode byte-identity guarantee together with its `DEBUG`-level *baggage*-leak limitation (CEL-003), and a "when to use each API" row for it; guidance on when to call `configure`, the three middlewares (`WSGIMiddleware`, `ASGIMiddleware`, `DjangoMiddleware`), `bind`, `inject`, `operation`, `flush` and `celery`; a before/after call-site pattern migration checklist; and a review checklist that a coding agent can apply mechanically. This document is the ONLY source of the agent-facing narrative content: `llms.txt` and `AGENTS.md` MUST link to it instead of duplicating its call-site rules or its checklists. On a broken installation (guide file missing), the resource error MUST propagate unmodified, instead of returning a fallback text that would mislead the agent ([AGENTSMD], [LLMSTXT]). Test: `tests/test_agent_docs_anti_drift.py`.

(Previously: no Celery integration route, and the API list before `celery` ended at `flush`.)

(Previously: "the two middlewares"; no Django `settings.MIDDLEWARE` recipe; the FastAPI `add_middleware` recipe was not required by this requirement's text.)

(Previously: required one recipe per framework, with no Django WSGI/ASGI wrapper route and no note on its exception visibility, and said nothing about hybrid's root logger level.)

(Previously: covered call-site rules for the pre-existing API only; now also covers `mode` and the `semlog=True` keyword.)

**DOC-012**: README.md and README.es.md MUST document, in parity (DOC-002): the three modes and their resolution precedence (LM-001); the `semlog=True` keyword and its cross-mode guarantees (LM-002); the hybrid non-stream-handler limitation (LM-004); `off` mode as the rollback switch, including the documented caveat that uninstalling `semlog` while marked call sites remain in the code raises `TypeError` at the call site rather than failing silently; the consequence of `hybrid` leaving the root logger's level exactly as found (LP-010), namely that a process which has not set a root level of its own filters out every `INFO` record marked `semlog=True` under the standard library's ordinary effective-level rules (LM-003), including the `INFO` `http.server.request` completion event of `log_requests=True`, that `configure(level=...)` does not change this because it applies in `full` mode only, and the remedy the application applies instead; and EVERY supported integration route per framework, each with a complete, runnable example and with the note saying which route to choose: for FastAPI and any other ASGI 3.0 application, `app.add_middleware(semlog.ASGIMiddleware, ...)` with its exception-handler caveat and the wrapper recipe whose routes are registered before the wrapping line; for Django, the `settings.MIDDLEWARE` recipe with `configure()` placed in `AppConfig.ready()` rather than `settings.py`, the `WSGIMiddleware`/`ASGIMiddleware` wrapper around `get_wsgi_application()`/`get_asgi_application()` with the note that Django converts a view exception into a 500 response before the wrapper sees it, so that route cannot attach the traceback, and the unsupported combination of the two (HTM-010); and for Celery, `semlog.celery(app)` called once in the module that creates `app`, restating that Django keeps `configure()` in `AppConfig.ready()` rather than in that module, the `full`-mode worker/beat/task logging guarantee, and the `hybrid`-mode byte-identity guarantee together with its `DEBUG`-level *baggage*-leak limitation (CEL-003). `llms.txt`'s optional short usage paragraph (DOC-009), if present, MUST stay consistent with this content. Test: `tests/test_readme_i18n.py::ReadmeModesParityTests`, `::ReadmeHybridRootLevelTests`, `::ReadmeFrameworkRecipesParityTests`.

(Previously: no Celery integration route documented.)

(Previously: required the modes, the keyword, the hybrid non-stream-handler limitation and `off` as the rollback switch only; said nothing about hybrid's root logger level, and named no integration route.)

> **Design decision (DOC-012).** Rationale: extends DOC-002's existing README parity policy to this change's new user-facing surface, so the two languages and the agent-facing files never diverge on how to adopt or roll back the feature, and a reader choosing between two supported integration routes is told what each one costs instead of finding out from a missing traceback in production. Rejected alternative: leaving mode/keyword documentation solely to STANDARDS.md and the agent guide, and documenting one route per framework, which would leave human adopters reading only README.md without any parity guarantee, risk README.md and README.es.md diverging silently, and leave the other supported route undocumented.

**DOC-013**: CHANGELOG.md MUST include a `## [0.5.0]` section, in Keep a Changelog format ([KEEPACHANGELOG], CP-004), dated, with a non-empty `### Added` subsection covering `semlog.celery(app)` and a non-empty `### Changed` subsection covering the CP-015 growth to eleven public names. The prior `## [0.4.0]` section MUST remain, unchanged, below it, and MUST still carry its own non-empty `### Added` subsection covering the `AiohttpMiddleware` class and the `aiohttp-matrix` continuous integration job, and its own non-empty `### Changed` subsection covering the CP-015 growth to ten public names; the `## [0.3.0]` section below that one MUST remain, unchanged, with its own non-empty `### Added` subsection covering the `DjangoMiddleware` class and the FastAPI `add_middleware` documentation, and its own non-empty `### Changed` subsection covering the CP-015/CP-017 amendments; and the `## [0.2.0]` section below that one MUST likewise remain, unchanged, with its own non-empty `### Added` subsection covering the three modes and the `semlog=True` keyword, and its own non-empty `### Fixed` subsection covering the writer stream-resolution and dead-writer-detection fixes (LP-006, LP-011). Test: `tests/test_packaging_hygiene.py::Changelog020Tests`, `::Changelog030Tests`, `::Changelog040Tests`, `::Changelog050Tests`.

(Previously: required only `## [0.4.0]`, with `## [0.3.0]` and `## [0.2.0]` below it; no `## [0.5.0]`.)

(Previously: required only `## [0.3.0]`, with `## [0.2.0]` below it; no `## [0.4.0]`.)

(Previously: required only `## [0.2.0]`, with `### Added` covering the three modes and `semlog=True`, and `### Fixed` covering LP-006/LP-011; did not require `## [0.3.0]` at all.)
...
| HTM-008 | 8.2 | Design decision | `tests/test_django_matrix.py::DjangoMiddlewareRegistrationTests`, `::DjangoMiddlewareAsyncMarkingTests`, `::DjangoMiddlewareModeTests`, `tests/test_packaging_hygiene.py::SemlogImportsWithoutDjangoTests` |
| HTM-009 | 8.2 | Design decision | `tests/test_readme_i18n.py::ReadmeFrameworkRecipesParityTests`, `tests/test_agent_docs_anti_drift.py` |
| HTM-010 | 8.2 | Design decision | `tests/test_django_matrix.py::DjangoMiddlewareCoexistenceTests` |
| HTM-011 | 8.2 | External: [OTEL-SEMCONV] | `tests/test_django_matrix.py::DjangoMiddlewareExceptionTests`, `::DjangoLogRequestsSettingTests` |
| HTM-012 | 8.2 | Design decision | `tests/test_django_matrix.py::DjangoMiddlewareExceptionVisibilityTests` |
| HTM-013 | 8.2 | Design decision | `tests/test_django_matrix.py::DjangoMiddlewareStreamingTests`, `::DjangoMiddlewareAsyncStreamingTests`, `::DjangoMiddlewareFileResponseTests` |
| HTM-014 | 8.2 | External: [PEP567] | `tests/test_aiohttp_middleware.py::AiohttpMiddlewareContextTests` |
| HTM-015 | 8.2 | Design decision | `tests/test_aiohttp_middleware.py::AiohttpMiddlewareRequestEventTests`, `::AiohttpHybridRequestEventTests` |
| HTM-016 | 8.2 | Design decision | `tests/test_aiohttp_middleware.py::AiohttpMiddlewareOffModeTests` |
| HTM-017 | 8.2 | Design decision | `tests/test_aiohttp_middleware.py::AiohttpEventDurationScopeTests` |
| CEL-001 | 8.4 | Design decision | `tests/test_celery_wiring.py::ReceiversNeverRaiseTests`, `tests/test_celery_integration.py::EntryPointIdempotencyTests`, `tests/test_packaging_hygiene.py::SemlogImportsWithoutCeleryTests` |
| CEL-004 | 8.4 | Design decision | `tests/test_celery_wiring.py::OffModeReceiverTests`, `tests/test_celery_integration.py::OffModeCeleryTests` |
| CEL-005 | 8.4 | External: [TRACECONTEXT], [PEP567] | `tests/test_celery_wiring.py::CarrierTests`, `::TaskSpanReceiverTests`, `tests/test_celery_integration.py::WorkerSpanTests`, `::EagerSpanTests` |
| CEL-006 | 8.4 | External: [TRACECONTEXT] | `tests/test_celery_wiring.py::PublishReceiverTests`, `tests/test_celery_integration.py::PublishInjectTests` |
| CEL-002 | 8.4 | Design decision | `tests/test_celery_wiring.py::FullModeFlagTests`, `::TaskLoggerCaptureTests`, `tests/test_celery_integration.py::ImportModulesFlagTests`, `tests/test_celery_matrix.py::FullModeWorkerTests`, `::FullModeBeatTests` |
| CEL-003 | 8.4 | Design decision | `tests/test_celery_wiring.py::HybridInertTests`, `tests/test_celery_matrix.py::HybridByteIdentityTests` |
| CEL-007 | 8.4 | External: [QUEUEHANDLER] | `tests/test_celery_wiring.py::ShutdownFlushTests`, `tests/test_celery_integration.py::ChildFlushWiringTests`, `tests/test_celery_matrix.py::PreforkShutdownFlushTests`, `::SoloExitFlushTests` |
...
| DOC-011 | 11.2 | External: [AGENTSMD], [LLMSTXT] | 1.10, 3.15, 3.16, 4.16, 4.18, 4.20, 4.21, 4.22, 4.23; `tests/test_agent_docs_anti_drift.py` |
| DOC-012 | 11.2 | Design decision | `tests/test_readme_i18n.py::ReadmeModesParityTests`, `::ReadmeHybridRootLevelTests`, `::ReadmeFrameworkRecipesParityTests` |
| DOC-013 | 11.2 | External: [KEEPACHANGELOG] | `tests/test_packaging_hygiene.py::Changelog020Tests`, `::Changelog030Tests`, `::Changelog040Tests` |
...
| CP-020 | 11 | External: [PYVERSIONS] | `tests/test_repository_policy.py::CeleryMatrixJobTests`, `tests/test_celery_matrix.py` |
| CP-014 | 11.1 | External: [PEP632], [PEP517], [PEP518] | 4.15 |
| CP-015 | 10 | External: [SEMVER] | 3.17, 4.3, 2.39, 2.40, `tests/test_public_surface.py` |
| CP-016 | 10 | Design decision | 2.29, 2.30 |
| CP-017 | 10 | Design decision | `tests/test_class_budget.py` |
