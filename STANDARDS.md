...
**DOC-013**: CHANGELOG.md MUST include a `## [0.3.0]` section, in Keep a Changelog format ([KEEPACHANGELOG], CP-004), dated, with a non-empty `### Added` subsection covering the `DjangoMiddleware` class and the FastAPI `add_middleware` documentation, and a non-empty `### Changed` subsection covering the CP-015/CP-017 amendments. The prior `## [0.2.0]` section MUST remain, unchanged, below it, and MUST still carry its own non-empty `### Added` subsection covering the three modes and the `semlog=True` keyword, and its own non-empty `### Fixed` subsection covering the writer stream-resolution and dead-writer-detection fixes (LP-006, LP-011). Test: `tests/test_packaging_hygiene.py::Changelog020Tests`, `::Changelog030Tests`.
...
| DOC-013 | 11.2 | External: [KEEPACHANGELOG] | `tests/test_packaging_hygiene.py::Changelog020Tests`, `::Changelog030Tests` |
