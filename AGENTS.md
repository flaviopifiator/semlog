...
- **Stdlib only, by default.** Any tool used in development, testing, building, or benchmarking must be stdlib, one of the explicitly approved third-party test/benchmark subjects (FastAPI, Starlette, Django, aiohttp, Celery, loguru, structlog), `ruff` (lint and format), GitHub Actions (CI), or the approved build backend (`uv_build`) and its build frontend (the `uv` CLI). `mypy`, `coverage.py`, and `jsonschema` are explicitly not approved. Anything else needs explicit maintainer approval before use; never adopt a new third-party package silently. Celery is a test-only subject (STANDARDS.md CP-008/CP-013): none of its own dependencies (for example its broker-client library) are separately approved, named, or pinned anywhere in this project.
- **Zero runtime dependencies.** The built package must always declare zero runtime (non-dev) dependencies, regardless of any development, testing, or benchmarking tooling choice. That includes package extras: an approved test subject is installed by the test job, never declared under `[project.optional-dependencies]`, so the wheel keeps zero `Requires-Dist` entries. A framework integration therefore imports its framework lazily, inside the method that needs it, never at `semlog` import time.
...
- The only approved third-party test and benchmark subjects are FastAPI, Starlette, Django, aiohttp, Celery, loguru, structlog: FastAPI, Starlette, Django, aiohttp and Celery for the compatibility matrix, loguru and structlog for the benchmark comparison. Every one of them is installed by its own CI matrix job with `uv run --with`, and none is ever declared in `pyproject.toml`, as a runtime dependency or as a package extra. Celery is approved as a test subject only; none of its own dependencies are separately approved, named, or pinned.
