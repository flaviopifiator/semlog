...
- The only approved third-party test and benchmark subjects are FastAPI, Starlette, Django, aiohttp, loguru, structlog: FastAPI, Starlette, Django and aiohttp for the compatibility matrix, loguru and structlog for the benchmark comparison. Every one of them is installed by its own CI matrix job with `uv run --with`, and none is ever declared in `pyproject.toml`, as a runtime dependency or as a package extra.
