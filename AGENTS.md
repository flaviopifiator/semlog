...
- **Source-line budget.** At most 1,200 non-blank, non-comment lines of package source, 6 classes, 0 abstract classes, and exactly 8 public names. The budget excludes tests, the bundled UUIDv7 fallback, and package data (including the agent guide). It is a guardrail against unbounded growth, not a design goal: flatness is already guaranteed by the class budget and the three-stage architecture (CP-016), not by the line count. Any growth in public surface or class count is a deliberate, documented, user-approved change.
